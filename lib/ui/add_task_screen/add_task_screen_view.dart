...
class _AddTaskScreenViewState extends State<AddTaskScreenView> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (context) => AddTaskScreenViewModelCubit(),
      child: Builder(
        builder: (context) {
          return Form(
            key: formKey,
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      CustomDropDownButtonFormField(
                        ...
                        selected: context.read<AddTaskScreenViewModelCubit>().selectedStatus,
                        onChange: (value) {
                          context.read<AddTaskScreenViewModelCubit>().selectedStatus = value!;
                        },
                      ),
                      SizedBox(height: height * 0.01),
                      CustomDropDownButtonFormField(
                        options: ["Low", "Medium", "High"],
                        selected: context.read<AddTaskScreenViewModelCubit>().selectedPriority,
                        onChange: (value) {
                          context.read<AddTaskScreenViewModelCubit>().selectedPriority = value!;
                        },
                      ),
                      SizedBox(height: height * 0.02),
                      Customtextformfield(
                        ...
                        onTap: () async {
                          ...
                          if (picked != null) {
                            widget.dateController.text = picked.toString().split(
                              " ",
                        ...
                        child: CustomElevatedButton(
                          content: Row(
                            ...
                            children: [
                              ...
                              Text("Save Task", style: AppFonts.bold16White),
                            ],
                          ),
                          onPress: () {
                            if(formKey.currentState!.validate()){
                              context.read<AddTaskScreenViewModelCubit>().addTask(
                                widget.sharedPrefServices.token!,
                                TaskResponse(
                                  title: widget.titleController.text,
                                  description: widget.descriptionController.text,
                                  status: context.read<AddTaskScreenViewModelCubit>().selectedStatus,
                                  priority:context.read<AddTaskScreenViewModelCubit>().selectedPriority,
                                  dueDate: widget.dateController.text,
                                  assignedTo: widget.sharedPrefServices.id,
                                ),
                              );
                            }
                          },
